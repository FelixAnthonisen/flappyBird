...
def circle_overlaps_rectangle(x0, y0, x1, y1, x2, y2, r):
    ...
    if (x2 >= left and x2 <= right) or (y2 >= top and y2 < bottom):
        return True
    return distance_between_points(x2, y2, x0, y0) < r or distance_between_points(x2, y2, x1, y0) < r or distance_between_points(x2, y2, x0, y1) < r or distance_between_points(x2, y2, x1, y1) < r


#obj1 has to be a rectangle and obj2 has to be a circle
def is_colliding(obj1, obj2):
    x0, y0, x1, y1 = (obj1.x_pos, obj1.y_pos, obj1.x_pos+obj1.width, obj1.y_pos+obj1.height)
